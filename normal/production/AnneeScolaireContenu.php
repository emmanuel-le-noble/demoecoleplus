<!DOCTYPE html>
<html lang="en">
<head></head>
    <body class="nav-md">
        <div class="right_col" role="main">
            <div class="">
                <div class="clearfix"></div>
				<div class="row">
				    <div class="col-md-12">
					    <div class="x_panel" style="box-shadow: 8px 8px 0px #aaa;">
					        <div class="x_title">
						        <h2 style="font-family:comic sans ms;font-weight:bold;">
						        	Ann&eacute;e scolaire
						        	<small style="color:#000;font-weight:bold;">
										[Permet de g&eacute;rer les ann&eacute;es scolaires]
									</small>
						        </h2>
						        <div class="clearfix"></div>
					        </div>
						    <div class="x_content">					    
								<div class="clearfix"></div>
								    <div style="margin-bottom:25px;">
										<span style="color:#000;font-weight:bold;font-size:14px;text-shadow: 0px 0px 4px rgba(0,0,0,0.75);font-family:comic sans ms">
										Les types d'op&eacute;rations &agrave; effectuer : </span>
										<input type="submit" style="width:90px;padding:2px;box-shadow: 8px 8px 0px #aaa;" class="btn btn-round btn-success" name="Ajouter" value="Ajouter"/>
										&nbsp;
										<input type="submit" style="width:90px;padding:2px;box-shadow: 8px 8px 0px #aaa;"class="btn btn-round btn-danger" name="Supprimer" value="Supprimer"
										onclick="return confirm('&ecirc;tes-vous s&ucirc;r de vouloir supprimer cette ann&eacute;e scolaire ?');"/>
										&nbsp;
										<input type="submit" style="width:90px;padding:2px;box-shadow: 8px 8px 0px #aaa;" class="btn btn-round btn-warning" name="Cloturer" value="Cl&ocirc;turer" onclick="return confirm('&ecirc;tes-vous s&ucirc;r de vouloir cl&ocirc;turer cette ann&eacute;e scolaire ?');"/>
										&nbsp;
										<input type="submit" style="width:110px;padding:2px;box-shadow: 8px 8px 0px #aaa;" class="btn btn-round btn-primary" name="Annuler" value="Annuler Cl&ocirc;ture" onclick="return confirm('&ecirc;tes-vous s&ucirc;r de vouloir annuler cette ann&eacute;e scolaire ?');"/>
										&nbsp;
									</div>
									<?php 
										if($error!="")
									    {
											?><div class="alert alert-warning alert-dismissible fade in" role="alert" style="width:100%;padding:5px;"><b><?php echo $error;?>
											</b></div><?php
	                                    }
	                                    elseif($success!="")
	                                    {
											?><div class="alert alert-success alert-dismissible fade in" role="alert" style="width:100%;padding:5px;border:1px solid #A2C600;"><b><?php echo $success;?>
											</b></div><?php
	                                    }
										ListeAnneeScolaire($pdo);
									?>
							</div>
						</div>
				    </div>
				</div>
            </div>
        </div>
    </body>
</html>